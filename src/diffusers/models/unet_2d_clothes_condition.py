...
class UNet2DConditionClothesLatentsModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    r"""
    ...
    def forward(
        ...
        clothes_latent = torch.permute(self.silu(self.clothes_to_encoder_1(clothes_latent)), (0, 2, 1))
        encoder_hidden_states = self.silu(self.clothes_to_encoder_2(clothes_latent))
        ...
        if self.config.center_input_sample:
            for idx in range(len(model_samples)):
                model_samples[idx] = 2 * model_samples[idx] - 1.0
        ...
        for idx, sample in enumerate(model_samples):
            # 2. pre-process
...
#########################################################################################################################################
class UNet2DConditionClothesInterpLatentsModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    r"""
    UNet2DConditionModel is a conditional 2D UNet model that takes in a noisy sample, conditional state, and a timestep
    and returns sample shaped output.
    ...
    def __init__(
        ...
        # input
        conv_in_padding = (conv_in_kernel - 1) // 2
        self.clothes_to_encoder_1 = nn.Linear(768,77)
        self.silu = nn.SiLU()
        self.conv_in = nn.Conv2d(
            in_channels, block_out_channels[0], kernel_size=conv_in_kernel, padding=conv_in_padding
        )
    ...
    def forward(
        ...
        model_samples = [sample, clothes_sample] if clothes_sample is not None else [sample]
        del encoder_hidden_states
        clothes_latent = F.interpolate(clothes_latent, size=384)
        clothes_latent = clothes_latent.reshape((2, 768, 768))
        clothes_latent = torch.permute(self.clothes_to_encoder_1(clothes_latent), (0, 2, 1))
        encoder_hidden_states = self.silu(clothes_latent)
...
#########################################################################################################################################
class UNet2DConditionClothesConvLatentsModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    r"""
    UNet2DConditionModel is a conditional 2D UNet model that takes in a noisy sample, conditional state, and a timestep
    and returns sample shaped output.
    ...
    def __init__(
        ...
        # input
        conv_in_padding = (conv_in_kernel - 1) // 2
        self.clothes_conv_1 = nn.ConvTranspose2d(4, 4, 4, 2, 1) 
        self.clothes_conv_2 = nn.ConvTranspose2d(4, 4, 3, 3, 1, dilation=2) 
        self.clothes_to_encoder_1 = nn.Linear(768, 77)
        self.silu = nn.SiLU()
        self.conv_in = nn.Conv2d(
            in_channels, block_out_channels[0], kernel_size=conv_in_kernel, padding=conv_in_padding
    ...
    def forward(
        ...
        del encoder_hidden_states
        clothes_latent = self.silu(self.clothes_conv_1(clothes_latent))
        clothes_latent = self.silu(self.clothes_conv_2(clothes_latent)).reshape((2, 768, 768))
        clothes_latent = torch.permute(self.clothes_to_encoder_1(clothes_latent), (0, 2, 1))
        encoder_hidden_states = self.silu(clothes_latent)
